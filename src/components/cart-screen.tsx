...
export function CartScreen() {
  ...
  const itemLabel = locale === "ru" ? (items.length === 1 ? "товар" : items.length < 5 ? "товара" : "товаров") : (items.length === 1 ? "product" : "products");
  return <div className="screen cart-screen"><header className="screen-header"><button className="icon-button" onClick={() => navigate("home")} aria-label={locale === "ru" ? "Назад" : "Back"}><ArrowLeft/></button><div><h1>{c.cartTitle}</h1><small>{items.length ? `${items.length} ${itemLabel}` : ""}</small></div>{items.length ? <button className="icon-button clear-cart-button" onClick={clearCart} aria-label={locale === "ru" ? "Очистить корзину" : "Clear cart"} title={locale === "ru" ? "Очистить корзину" : "Clear cart"}><Trash2/></button> : <span/>}</header>
    {!items.length ? <div className="empty-state cart-empty"><ShoppingBag size={28}/><h2>{c.emptyCart}</h2><p>{c.emptyCartBody}</p><button className="primary-button" onClick={() => navigate("assistant")}><Sparkles size={15}/>{c.tryIt}</button><button className="secondary-button" onClick={() => navigate("catalog")}>{c.browse}</button></div> : <>
      <div className="cart-list">{items.map((item) => <article className="cart-row" key={item.product.id}><div className="cart-placeholder" aria-hidden/><div className="cart-info"><strong>{item.product.localeData[locale].name}</strong><small>{item.product.brand || "Market choice"}</small><b>{price(item.product.price)}</b>{item.reason ? <span className="ai-choice"><Sparkles size={11}/>{locale === "ru" ? "Выбор AI" : "AI choice"}</span> : null}</div><QuantityControl quantity={item.quantity} removeAtOne onDecrease={() => setQuantity(item.product.id, item.quantity - 1)} onIncrease={() => setQuantity(item.product.id, item.quantity + 1)} disabledIncrease={item.quantity >= item.product.stock}/></article>)}</div>
