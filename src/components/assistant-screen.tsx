...
import { ArrowLeft, Bot, Check, ChevronDown, ChevronRight, Clock3, CornerDownLeft, History, LoaderCircle, MessageSquarePlus, Plus, Send, ShieldCheck, Sparkles, Users } from "lucide-react";
...
const prompts = {
  en: ["Build a taco dinner for four under $45.", "I need a quick dairy-free dinner for two.", "Plan three weekday breakfasts with minimal leftovers.", "Make shrimp pasta for under $30.", "Build a picnic basket for three people."],
  ru: ["Собери шакшуку на двоих до $25.", "Хочу приготовить лёгкий ужин без молочных продуктов.", "Собери продукты для трёх завтраков на рабочую неделю.", "Сделай пасту с креветками дешевле $30.", "Собери салат на четверых, но без майонеза и орехов."],
};
...
export function AssistantScreen() {
  ...
  return <div className="screen assistant-screen">
    <header className="assistant-header"><button className="icon-button" aria-label={locale === "ru" ? "Назад" : "Back"} onClick={() => view === "home" ? navigate("home") : setView("home")}><ArrowLeft/></button><div className="assistant-avatar"><Bot size={19}/><span/></div><div><h1>{view === "history" ? (locale === "ru" ? "История чатов" : "Chat history") : c.assistantTitle}</h1><small>{view === "history" ? (locale === "ru" ? `${chatSessions.length} сохранено` : `${chatSessions.length} saved`) : c.assistantSub}</small></div><button className="assistant-history-button" disabled={assistantStatus === "running"} onClick={() => setView("history")} aria-label={locale === "ru" ? "История чатов" : "Chat history"}><History size={17}/></button></header>
    {view === "home" ? <div className="chat-scroll assistant-home-scroll">
      <div className="assistant-empty"><div className="big-assistant-icon"><Bot size={31}/></div><span className="eyebrow"><Sparkles size={13}/> Grocery Copilot</span><h2>{c.emptyChat}</h2><p>{c.emptyChatBody}</p>
        ...
        {messages.length ? <button className="continue-conversation" onClick={() => setView("chat")}><Bot size={15}/>{locale === "ru" ? "Продолжить диалог" : "Continue conversation"}</button> : null}
      ...
      {chatSessions.length ? <div className="chat-history-list">{[...chatSessions].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((chat) => <button key={chat.id} className={chat.id === sessionId ? "active" : ""} onClick={() => { openChat(chat.id); setView("chat"); }}><span className="chat-history-icon"><Bot size={16}/></span><span><strong>{chat.title}</strong><small>{chat.messages.length} {locale === "ru" ? "сообщений" : "messages"} · {chat.updatedAt.slice(0, 10)}</small></span><ChevronRight size={16}/></button>)}</div> : <div className="chat-history-empty"><History size={27}/><strong>{locale === "ru" ? "История пока пуста" : "No chats yet"}</strong><p>{locale === "ru" ? "Первый запрос появится здесь автоматически." : "Your first request will appear here automatically."}</p></div>}
    </div> : <>
